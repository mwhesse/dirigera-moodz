...
export class SyncEngine {
  ...
  private async createLightPattern(totalEnergy: number, bass: number, mids: number, treble: number): Promise<void> {
    ...
    const devices = this.dirigeraService.getDevices().filter(d => d.currentState.isOn && d.capabilities.canChangeColor && d.isSelected);
    
    this.logger.debug(`🔍 PATTERN CHECK: Found ${devices.length} lights that are ON, support color, and are selected`);
    devices.forEach(d => this.logger.debug(`✅ ACTIVE: ${d.name} (ON: ${d.currentState.isOn})`));
