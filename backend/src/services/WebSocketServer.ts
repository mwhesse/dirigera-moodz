...
export class WebSocketServer {
  ...
  private async handleMessage(clientId: string, message: WSMessage): Promise<void> {
    ...
    try {
      switch (message.type) {
        ...
        case 'PLAYBACK_STATE':
          await this.handlePlaybackState(message.data);
          this.broadcast({
  ...
  private async handlePlaybackState(data: any): Promise<void> {
    // Log playback state changes
    this.logger.info(`Playback state: ${data.isPlaying ? 'playing' : 'paused'} - ${data.trackId}`);
    
    // You could implement additional logic here, such as:
    // - Pausing light sync when music is paused
    // - Storing track information for analytics
    // - Adjusting sync parameters based on track metadata
  }
